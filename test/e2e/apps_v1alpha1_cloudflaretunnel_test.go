...
// appsv1alpha1CloudflareTunnel tests
func appsv1alpha1CloudflareTunnelChildrenFuncs(tester *E2ETest) error {
	// TODO: need to run r.GetResources(request) on the reconciler to get the mutated resources
	if len(cloudflared.CreateFuncs) == 0 {
		return nil
	}

	workload, err := cloudflared.ConvertWorkload(tester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := cloudflared.Generate(*workload, nil, nil)
	if err != nil {
...
func appsv1alpha1CloudflareTunnelNewHarness(namespace string) *E2ETest {
	return &E2ETest{
		namespace:          namespace,
		unstructured:       &unstructured.Unstructured{},
		workload:           &appsv1alpha1.CloudflareTunnel{},
		sampleManifestFile: "../../config/samples/apps_v1alpha1_cloudflaretunnel.yaml",
		getChildrenFunc:    appsv1alpha1CloudflareTunnelChildrenFuncs,
		logSyntax:          "controllers.apps.CloudflareTunnel",
	}
}

func (tester *E2ETest) appsv1alpha1CloudflareTunnelTest(testSuite *E2EComponentTestSuite) {
...
func (testSuite *E2EComponentTestSuite) Test_appsv1alpha1CloudflareTunnel() {
	tester := appsv1alpha1CloudflareTunnelNewHarness("")
	tester.appsv1alpha1CloudflareTunnelTest(testSuite)
}
