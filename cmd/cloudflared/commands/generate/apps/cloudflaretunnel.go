...
func NewCloudflareTunnelSubCommand(parentCommand *cobra.Command) {
	generateCmd := &cmdgenerate.GenerateSubCommand{
		...
		GenerateFunc: GenerateCloudflareTunnel, UseWorkloadManifest: true,
		WorkloadKind: "CloudflareTunnel",
	}

	generateCmd.Setup()
}

// GenerateCloudflareTunnel runs the logic to generate child resources for a
// CloudflareTunnel workload.
func GenerateCloudflareTunnel(g *cmdgenerate.GenerateSubCommand) error {
	...
	generateFuncMap := map[string]generateFunc{
		"v1alpha1": v1alpha1cloudflaretunnel.GenerateForCLI,
